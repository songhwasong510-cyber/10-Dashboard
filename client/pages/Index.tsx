        ...
        <div className="flex items-center gap-1.5 mb-6">
          <div className="relative inline-flex items-center gap-1 px-3 py-1.5 rounded border border-neutral-200 shadow-sm bg-white">
            ...
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" className="ml-1">
              <path d="M8.99949 9.87831L12.7118 6.16602L13.7725 7.22667L8.99949 11.9997L4.22656 7.22667L5.28722 6.16602L8.99949 9.87831Z" fill="#1D293D"/>
            </svg>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_455px] gap-[14px]">
          ...
          <div className="rounded-lg border border-neutral-200 shadow-sm bg-white">
            <div className="h-[46px] border-b border-neutral-200 px-5 flex items-center">
              <h2 className="text-base font-bold text-text-primary">비인가 에이전트 접근 시도</h2>
            </div>
            <div className="p-5">
              {/* Legend */}
              <div className="flex items-center justify-end gap-4 mb-4">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-bold text-text-primary">비인가 에이전트 접근 시도</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full bg-[#4F39F6]"></div>
                  <span className="text-xs font-bold text-text-primary">사용자(daily)</span>
                </div>
              </div>
              ...
              <div className="relative h-[310px]">
                ...
                <div className="absolute inset-0 flex flex-col justify-between">
                  {[...Array(9)].map((_, i) => (
                    <div key={i} className={`w-full ${i === 8 ? 'border-b border-[#90A1B9]' : 'border-b border-dashed border-[#90A1B9]'}`}></div>
                  ))}
                </div>
                ...
                <div className="absolute left-0 top-0 bottom-0 flex flex-col justify-between text-xs text-text-primary text-right pr-2 w-4">
                  {[8, 7, 6, 5, 4, 3, 2, 1, 0].map((val) => (
                    <span key={val} className="-mt-2">{val}</span>
                  ))}
                </div>
                ...
                {/* Purple line overlay */}
                <svg className="absolute left-8 right-0 bottom-8 top-0 pointer-events-none" style={{ overflow: 'visible' }}>
                  <polyline
                ...
                {chartData[9].isActive && (
                  <div className="absolute right-12 top-32 w-[155px] rounded bg-text-primary shadow-lg p-4 z-10">
                    <p className="text-xs font-bold text-white mb-1">비인가 에이전트 접근 시도</p>
                    <div className="flex items-baseline gap-1">
                      <span className="text-base font-bold text-white">12-12일 :</span>
                      <span className="text-[26px] font-bold text-white leading-none">1</span>
                    </div>
          ...
          <div className="rounded-lg border border-neutral-200 shadow-sm bg-white">
            <div className="h-[46px] border-b border-neutral-200 px-5 flex items-center">
              <h2 className="text-base font-bold text-text-primary">시스템 상태</h2>
            </div>
            <div className="p-5 flex items-center justify-center" style={{ height: 'calc(100% - 46px)' }}>
              <div className="relative w-[232px] h-[232px]">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <span className="text-4xl font-bold text-[#006045]">안전</span>
                  </div>
                </div>
              </div>
            </div>
